...
ggplot(mod1_resids_df) + 
  geom_sf(aes(fill = mod1_resids))  +
...
neighbours1 <- poly2nb(all_elections_polygons_simp_table, queen=T)
weights1 <- nb2listw(neighbours1, style="W", zero.policy = T)

all_elections_polygons_simp_table <- as.data.frame(all_elections_polygons_simp)
#######

locali <- localmoran(all_elections_polygons_simp_table[,298], weights1, zero.policy = TRUE)
...
m.lowqual.change <- all_elections_polygons_simp_table[,298] - mean(all_elections_polygons_simp_table[,298])     
...
legend("bottomleft", legend = c("insignificant","low-low","low-high","high-low","high-high"),
       fill=colors, bty="n")
title(main = paste("LISA Clusters: ", names(all_elections_polygons_simp_table)[298]))
...
# 
# # centers the variable of interest around its mean
# m.lowqual.change <- gwr_table1[,i] - mean(gwr_table1[,i])     
# 
# # centers the local Moran's around the mean
# m.local <- locali[,1] - mean(locali[,1])    
# 
...
# 
# # centers the variable of interest around its mean
# m.lowqual.change <- gwr_pca_table[,i] - mean(gwr_pca_table[,i])     
# 
# # centers the local Moran's around the mean
# m.local <- locali[,1] - mean(locali[,1])    
# 
