...
tb <- table(df$density_categs,df$con_flip)
tb1 <- round(prop.table(tb, margin=1), digits=2)
kable(tb1, table.attr = "style='width:40%;'" ) %>% kable_classic(full_width = T, position = "center", )
```
...
model1.4 <- lm(con_change1 ~ over65*density_categs + deprived*density_categs, data=df1)
summary(model1.4)

df2 <- df %>% 
  select(con_change1,over65,deprived,density,density_categs) %>% 
  mutate(density=log(density)) %>% 
  st_drop_geometry()
ggpairs(df2)+theme_classic()

ggplot(df2)+geom_smooth(aes(x=deprived,y=con_change1,colour=density_categs))+theme_classic()
ggplot(df2)+geom_point(aes(x=deprived,y=log(density),colour=con_change1))+theme_classic()+scale_color_continuous_diverging(rev=TRUE)
