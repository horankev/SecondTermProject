...
all_elections1 <- all_elections1 %>%
  mutate(over65 = age_65_to_74 + age_75_to_84 + age_85_to_89 + age_90_plus,
         ...
         density = population_density)
vars <- all_elections1 %>% 
  dplyr::select(con_change, low_qual, over65, deprived, student) %>% 
  st_drop_geometry()
```
...
vars_all <- all_elections1 %>% 
  dplyr::select(con_change, low_qual, over65, student, deprived, density, leave_EU, con_17, health_very_bad, house_owned, household_one_person, 
...
lin_mod_all <- lm(con_change ~ low_qual + over65 + student + deprived + density + leave_EU + 
                          con_17 + health_very_bad + house_owned + household_one_person + 
...
myColors <- c(brewer.pal(8, "Paired"), "grey") # take 11 from paired and add grey for not featured
...
vars2 <- all_elections1 %>% 
  dplyr::select(low_qual, over65, student, deprived, density, leave_EU, con_17, health_very_bad, house_owned, household_one_person, 
                ethnicity_white, unemployed, born_elsewhere) %>% st_drop_geometry()
...
coef_health_very_bad <- (v$loadings[8,1] * as.numeric(mod_pca$coefficients[2])) + 
  (v$loadings[8,2] * as.numeric(mod_pca$coefficients[3]))
coef_house_owned <- (v$loadings[9,1] * as.numeric(mod_pca$coefficients[2])) + 
  (v$loadings[9,2] * as.numeric(mod_pca$coefficients[3]))
coef_household_one_person <- (v$loadings[10,1] * as.numeric(mod_pca$coefficients[2])) + 
  (v$loadings[10,2] * as.numeric(mod_pca$coefficients[3]))
coef_ethnicity_white <- (v$loadings[11,1] * as.numeric(mod_pca$coefficients[2])) + 
  (v$loadings[11,2] * as.numeric(mod_pca$coefficients[3]))
coef_unemployed <- (v$loadings[12,1] * as.numeric(mod_pca$coefficients[2])) + 
  (v$loadings[12,2] * as.numeric(mod_pca$coefficients[3]))
coef_born_elsewhere <- (v$loadings[13,1] * as.numeric(mod_pca$coefficients[2])) + 
  (v$loadings[13,2] * as.numeric(mod_pca$coefficients[3]))



table <- as.data.frame(rbind(coef_low_qual,coef_over65,coef_student,coef_deprived,coef_density,coef_leave_EU,coef_cons17,
...
coef_health_very_bad_gw <- (v$loadings[8,1] * as.numeric(all_elections1$GWRcoef_PC1)) + 
  (v$loadings[8,2] * as.numeric(all_elections1$GWRcoef_PC2))
coef_house_owned_gw <- (v$loadings[9,1] * as.numeric(all_elections1$GWRcoef_PC1)) + 
  (v$loadings[9,2] * as.numeric(all_elections1$GWRcoef_PC2))
coef_household_one_person_gw <- (v$loadings[10,1] * as.numeric(all_elections1$GWRcoef_PC1)) + 
  (v$loadings[10,2] * as.numeric(all_elections1$GWRcoef_PC2))
coef_ethnicity_white_gw <- (v$loadings[11,1] * as.numeric(all_elections1$GWRcoef_PC1)) + 
  (v$loadings[11,2] * as.numeric(all_elections1$GWRcoef_PC2))
coef_unemployed_gw <- (v$loadings[12,1] * as.numeric(all_elections1$GWRcoef_PC1)) + 
  (v$loadings[12,2] * as.numeric(all_elections1$GWRcoef_PC2))
coef_born_elsewhere_gw <- (v$loadings[13,1] * as.numeric(all_elections1$GWRcoef_PC1)) + 
  (v$loadings[13,2] * as.numeric(all_elections1$GWRcoef_PC2))



temp_table <- cbind(coef_low_qual_gw, coef_over65_gw, coef_student_gw, coef_deprived_gw, 
               coef_density_gw, coef_leave_EU_gw, coef_cons17_gw,
...
spdf <- all_elections1 %>%
  mutate(over65 = age_65_to_74 + age_75_to_84 + age_85_to_89 + age_90_plus,
         ...
         density = population_density) %>%
  dplyr::select(low_qual, over65, student, deprived, density, leave_EU, con_17, health_very_bad, house_owned, household_one_person,
                ethnicity_white, unemployed, born_elsewhere, geometry) %>%
  as_Spatial()
...
# then use GWPCA components to perform GWR...??? not done properly yet...
### TEST SECTION
...
coef_cons17_gwpca[i] <- as.vector((loadings[i,7,1] * as.numeric(gwr.model.gwpca$SDF$PC1gw[i])) + 
...
gwr_gwpca_table <- as.data.frame(cbind(coef_low_qual_gwpca, coef_over65_gwpca, coef_student_gwpca, coef_deprived_gwpca,
               coef_density_gwpca, coef_leave_EU_gwpca, coef_cons17_gwpca,
...
newcols <- all_elections1[,327:339] %>% 
  st_drop_geometry() %>% 
  abs()
all_elections_polygons$highest_imp <- colnames(newcols)[apply(newcols,1,which.max)]
...
subtable <- all_elections1[,c(4,327:339)] %>% 
  st_drop_geometry() %>% 
  cbind(all_elections_polygons$geometry) %>%  pivot_longer(2:14) %>% 
...
myColors <- c(brewer.pal(7, "Paired"), "grey") # take 7 from paired and add grey for not featured
...
subtable1$fill <- as.factor(subtable1$fill)
myColors <- c(brewer.pal(11, "Paired"), "grey") # take 11 from paired and add grey for not featured
names(myColors) <- levels(subtable1$fill)
...
r2.lm <- glance(lin_mod_all)$r.squared


r2.gwr <- as.data.frame(gwr.model$SDF$localR2)
r2.gwr.gwpca <- as.data.frame(gwr.model.gwpca$SDF$localR2)
r2.gwr.comparison <- as.data.frame(gwr.model_comparison$SDF$localR2)

r2_combined <- cbind(r2.gwr, 
                     r2.gwr.gwpca, 
                     r2.gwr.comparison,
                     all_elections_polygons$geometry) %>% 
  st_as_sf()

pr1 <- ggplot(r2_combined) + geom_sf(aes(fill=`gwr.model$SDF$localR2`)) +
  coord_sf(datum = NA) +  
  theme_light() +
  scale_fill_gradient2(low="red",mid="white",high="blue", midpoint = 0.5) + 
  labs(title = "R2 GWR global PCA")
pr2 <- ggplot(r2_combined) + geom_sf(aes(fill=`gwr.model.gwpca$SDF$localR2`)) +
  coord_sf(datum = NA) +  
  theme_light() +
  scale_fill_gradient2(low="red",mid="white",high="blue", midpoint = 0.5) + 
  labs(title = "R2 GWR local PCA")
pr3 <- ggplot(r2_combined) + geom_sf(aes(fill=`gwr.model_comparison$SDF$localR2`)) + 
  coord_sf(datum = NA) + 
  theme_light() +
  scale_fill_gradient2(low="red",mid="white",high="blue", midpoint = 0.5) + 
  labs(title = "R2 GWR census variables")

ggarrange(pr1, pr2, pr3, ncol=3, nrow=1, common.legend = TRUE, legend="bottom") + 
  labs(title = "R2 of Differents GWR Models")
```
...
```{r, fig.height=12, fig.width=12}
ggparcoord(all_elections1, columns = 327:339, groupColumn = 6, scale = "center") + 
  custom_colors + 
  facet_wrap(~region) + 
  theme(legend.position="none") + 
  coord_flip()
...
all_elections_temp <- all_elections1 %>% 
  dplyr::select(327:339) %>% 
  st_drop_geometry() %>% 
  abs() %>% 
  cbind(all_elections1[,6])

ggparcoord(all_elections_temp, columns = 1:13, groupColumn = 14) + 
  custom_colors + 
  facet_wrap(~region) + 
...
p_w <- ggparcoord(all_elections_temp %>% filter(region=="Wales"), columns = 1:13, scale = "center")
p_se <- ggparcoord(all_elections_temp %>% filter(region=="South East"), columns = 1:13, scale = "center")
p_wm <- ggparcoord(all_elections_temp %>% filter(region=="West Midlands"), columns = 1:13, scale = "center")
p_nw <- ggparcoord(all_elections_temp %>% filter(region=="North West"), columns = 1:13, scale = "center")
p_em <- ggparcoord(all_elections_temp %>% filter(region=="East Midlands"), columns = 1:13, scale = "center")
p_l <- ggparcoord(all_elections_temp %>% filter(region=="London"), columns = 1:13, scale = "center")
p_yh <- ggparcoord(all_elections_temp %>% filter(region=="Yorkshire and The Humber"), columns = 1:13, scale = "center")
p_e <- ggparcoord(all_elections_temp %>% filter(region=="Wales"), columns = 1:13, scale = "center")
p_sw <- ggparcoord(all_elections_temp %>% filter(region=="South West"), columns = 1:13, scale = "center")
p_ne <- ggparcoord(all_elections_temp %>% filter(region=="North East"), columns = 1:13, scale = "center")
