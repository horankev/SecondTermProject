...
# response: Conservative Vote
### percentage of Conservative voters in 2019
...
ggplot(all_elections_polygons) + 
  geom_histogram(aes(con_change1),binwidth = 10, fill="#AFC1CC", colour="#517281") + 
  xlab("Proportion") + 
  ylab("Number of Constituencies") + 
  labs(title = "Percentage of Voters Who Voted Conservative", 
       subtitle = "2019 elections")
```
...
cons_constit <- all_elections_polygons %>%
  ...
  group_by(county) %>%
  summarise(mean_con = weighted.mean(con_change1,electorate_19))
...
pl1 <- ggplot(cons_constit,aes(fill=fitted)) + geom_sf(lwd=0.05) +
...
cc_CAR <- gam(con_change1 ~ low_qual + over65 + student + deprived + density + leave_EU + 
              ...
              weight=electorate_19, method='REML')
summary(cc_CAR)
...
plot1 <- ggplot(counties_simp,aes(fill=agamma_i)) + geom_sf() +
...
plot2 <- ggplot(counties_simp,aes(fill=gamma_i)) + geom_sf() +
...
b1 <- ggplot(counties_simp,aes(fill=agamma_i)) + geom_sf() +
...
#   summarise(low_qual = weighted.mean(low_qual,electorate_19), 
#             over65 = weighted.mean(over65,electorate_19), 
#             student = weighted.mean(student,electorate_19), 
#             deprived = weighted.mean(deprived,electorate_19), 
#             density = weighted.mean(density,electorate_19), 
#             leave_EU = weighted.mean(leave_EU,electorate_19), 
#             house_owned = weighted.mean(house_owned,electorate_19), 
#             unemployed = weighted.mean(unemployed,electorate_19), 
#             born_elsewhere = weighted.mean(born_elsewhere,electorate_19)
#             )
...
b2 <- ggplot(counties_simp,aes(fill=gamma_i)) + geom_sf() +
  scale_fill_distiller(type='div',
                       name=expression(gamma[leave_EU]))
b3 <- ggplot(counties_simp,aes(fill=slope)) + geom_sf() +
  scale_fill_distiller(type='div',
                       name=("Slope:\nleave_EU"))
...
ggplot(counties_simp3,aes(fill=agamma_i)) + geom_sf() +
  scale_fill_distiller(name=expression(alpha+gamma[0]),direction=1)
...
counties_simp3 <- counties_simp3 %>% 
  mutate(gamma_i=predict(cc_CAR3,
         ...
         slope = gamma_i / weighted.mean(all_elections_polygons$leave_EU,all_elections_polygons$electorate_19))
...
v1 <- ggplot() + geom_sf(data=counties_simp3,aes(fill=slope_low_qual),lwd=NA) +
  scale_fill_continuous_diverging(palette = 'Vik', mid = 0,
                       name="Slope") + 
  geom_sf(data=counties_simp, alpha=0,lwd=.45, colour="black") + 
  labs(title = "Low Qualifications")
